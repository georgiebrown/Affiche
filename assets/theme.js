...
let
  variantSlideshowSelecotr = '.js-variant-slideshow',
  variantSlideshows = {
    setup: function($element){
      let 
        $slides = $element.find(slideSelector),
        variantSlideshowOptions = {
          fade: true,     
        };
      $slides.slick(variantSlideshowOptions);

    },
    init: function(){
      $(variantSlideshowSelector).each(function(){
        variantSlideshows.setup($(this));
      });
    }
...
var brownButton = document.querySelectorAll('.color-c29070');

brownButton.forEach(button => {
  button.addEventListener('click', event => {
    
    var revealContainer = event.target.closest('.reveal').children;
    var closestBrownImage = revealContainer[0];
    var closestBlackImage = revealContainer[1];
    var closestWhiteImage = revealContainer[2];
    closestBlackImage.style.opacity = '0';
    closestWhiteImage.style.opacity = '0';
    closestBrownImage.style.opacity = '100';
    
    console.log('clicked')
  })
});

var blackButton = document.querySelectorAll('.color-000000');

blackButton.forEach(button => {
  button.addEventListener('click', event => {
    var revealContainer = event.target.closest('.reveal').children;
    var closestBlackImage = revealContainer[1];
    var closestBrownImage = revealContainer[0];
    ...
    closestWhiteImage.style.opacity = '0';
    closestBrownImage.style.opacity = '0';
    closestBlackImage.style.opacity = '100';

    console.log('clicked')
  })
});
var whiteButton = document.querySelectorAll('.color-FFFFFF');
...
whiteButton.forEach(button => {
  button.addEventListener('click', event => {
    ...
    closestBlackImage.style.opacity = '0';
    closestBrownImage.style.opacity = '0';
    closestWhiteImage.style.opacity = '100';
